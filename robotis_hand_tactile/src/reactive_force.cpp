  ...
  step_count_(0)
{
  force_topic_ = this->declare_parameter<std::string>(
    "force_topic", "/tactile_force_per_finger");
...
//   min_contact_force_["little"] = this->declare_parameter<double>("min_contact_force.little", 5.0);
  ...
  // sensitive 용
  delta_threshold_["thumb"]  = this->declare_parameter<double>("delta_threshold.thumb", 2.0);
  delta_threshold_["index"]  = this->declare_parameter<double>("delta_threshold.index", 2.0);
  delta_threshold_["middle"] = this->declare_parameter<double>("delta_threshold.middle", 2.0);
  delta_threshold_["ring"]   = this->declare_parameter<double>("delta_threshold.ring", 2.0);
  delta_threshold_["little"] = this->declare_parameter<double>("delta_threshold.little", 2.0);

  variation_threshold_["thumb"]  = this->declare_parameter<double>("variation_threshold.thumb", 1.0);
  variation_threshold_["index"]  = this->declare_parameter<double>("variation_threshold.index", 1.0);
  variation_threshold_["middle"] = this->declare_parameter<double>("variation_threshold.middle", 1.0);
  variation_threshold_["ring"]   = this->declare_parameter<double>("variation_threshold.ring", 1.0);
  variation_threshold_["little"] = this->declare_parameter<double>("variation_threshold.little", 1.0);

  min_contact_force_["thumb"]  = this->declare_parameter<double>("min_contact_force.thumb", 1.0);
  min_contact_force_["index"]  = this->declare_parameter<double>("min_contact_force.index", 1.0);
  min_contact_force_["middle"] = this->declare_parameter<double>("min_contact_force.middle", 1.0);
  min_contact_force_["ring"]   = this->declare_parameter<double>("min_contact_force.ring", 1.0);
  min_contact_force_["little"] = this->declare_parameter<double>("min_contact_force.little", 1.0);
...
void ReactiveForceNode::forceCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
{
  ...
  if (msg->data.size() < finger_names_.size()) {
    RCLCPP_WARN(
      this->get_logger(),
      "Force array too small. expected=%zu, got=%zu",
      finger_names_.size(), msg->data.size());
    return;
  }
  ...
  for (size_t i = 0; i < finger_names_.size(); ++i) {
    ...
    double force = static_cast<double>(msg->data[i]);
    ...
    if (trigger) {
      ...
      RCLCPP_INFO(
        ...
        finger.c_str(), force, avg, d, var, sl);
    }
  }

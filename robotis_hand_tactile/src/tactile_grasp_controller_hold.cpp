...
TactileGraspController::TactileGraspController() : Node("tactile_grasp_controller") {
  init_finger_configs();
  init_joint_name_list();

  tactile_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
      "tactile_force", 10, std::bind(&TactileGraspController::tactile_callback, this, std::placeholders::_1));
...
// tactile_force topic callback
void TactileGraspController::tactile_callback(const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
  if (msg->data.size() < 15) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "tactile_force size is smaller than 15");
    return;
  }
  ...
  for (int i = 0; i < k_num_fingers; ++i) {
    ...
    fingers_[i].filtered_force = alpha_ * raw + (1.0 - alpha_) * fingers_[i].filtered_force;
  }
}
